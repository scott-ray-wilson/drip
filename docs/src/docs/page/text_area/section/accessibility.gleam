...
fn content() -> Block(message) {
  content.Group([
    ...
    content.Callout(title: "Native semantics, validation aware", body: [
      content.Text("Renders as a native "),
      content.Code("<textarea>"),
      content.Text(
        " with full keyboard support, surfaces validation via aria-invalid, and
        uses field-sizing so the control grows with its content rather than
        masking overflow. Pair each control with a label element for screen
        readers.",
      ),
    ]),
    ...
    content.Table(headers: ["Key", "Action"], rows: [
      ...
      [
        ...
        content.text_cell("Insert a newline at the caret."),
      ],
    ]),
  ])
}
