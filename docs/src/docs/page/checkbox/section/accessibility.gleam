...
fn content() -> Block(message) {
  content.Group([
    ...
    content.Callout(title: "Native semantics, keyboard ready", body: [
      ...
      content.Text(
        ", so the browser handles toggling, focus, and form submission. Pair
        every checkbox with a field label so screen readers announce the option,
        and group related checkboxes inside a field set with a legend to bind a
        heading to the set.",
      ),
    ]),
    ...
    content.Table(headers: ["Key", "Action"], rows: [
      [
        ...
        content.text_cell("Move focus to the checkbox."),
      ],
      [
      ...
      [
        ...
        content.text_cell("Toggle the checked state."),
      ],
    ]),
  ])
}
