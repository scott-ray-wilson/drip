...
import docs/page/empty/example/introduction
import docs/ui/table_of_contents.{type Entry}
import lustre/element.{type Element}

// The page header renders at the page level (see empty/page.gleam) so it can
...
fn content() -> Block(message) {
  content.Example(code: empty.introduction_html, body: [
    introduction.view(),
  ])
}
